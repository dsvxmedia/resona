...
> **Status: template, not filled in yet.** This gets written for real as the last build step (see `PLAN-LITE.md`, step 12), after the actual application exists and has been tested. Right now this is the outline and skeleton so nothing gets forgotten. Whoever picks this project back up should fill in every `[TO FILL IN]` with what actually happened, in plain language, with no jargon left unexplained.
...
### 3c. Naming it
[TO FILL IN: why "Resona," what it means, and why it avoids using Influur's own product name "Pulse."]

## Part 4: The Building Blocks (technical decisions, explained for everyone)

For each item below: **what it is (plain English), why we picked it, and what would have happened if we picked something else.**

### 4a. "Agents": what does that even mean?
[TO FILL IN: explain an AI agent like a helpful robot coworker who can use tools (like looking things up, doing math, writing a message) instead of just chatting. Explain "multi-agent" as a team of these robot coworkers, each with one job, managed by a lead robot (the orchestrator) who decides what happens next.]

### 4b. Why more than one AI model?
[TO FILL IN: explain in plain terms that different AI models are good at different jobs, like how you'd hire a translator instead of an accountant to translate a document. Explain which model did which job and why, for example: "the smart but pricier model made the big decisions and looked at the video, while the fast and cheap model wrote several short messages quickly."]

### 4c. What is "RAG" and "embeddings"? (in plain English)
[TO FILL IN: explain embeddings as turning descriptions into a list of numbers that captures "meaning," so a computer can measure how similar two things are. It's a bit like a GPS coordinate, but for ideas instead of places. Explain RAG (Retrieval-Augmented Generation) as "look up the most relevant facts first, then let the AI use them to answer" instead of the AI just guessing from memory.]

### 4d. How did it understand a TikTok video?
[TO FILL IN: explain that the program grabbed the video's thumbnail picture and caption, and asked an AI that can "see" pictures to describe the video's style. Fast cuts or slow? High energy or chill? What kind of story is it telling? That got turned into structured tags, the same way a music supervisor would describe a scene.]

### 4e. How did it score and pick creators?
[TO FILL IN: explain the scoring was not done by AI guessing, but by a plain math formula the team could see and explain, like a report card with clear categories, not a mystery grade. Explain why: predictable, explainable, and cheaper and faster than asking an AI to do arithmetic.]
...
[TO FILL IN as a simple numbered list once built. Mirror the steps in PLAN-LITE.md's "Build sequencing" section, but rewritten in plain language, for example: "1. Set up the empty project skeleton. 2. Created a pretend list of about 35 TikTok creators with made-up but realistic profiles, since we don't have Influur's real data. 3. Built the 'brain' that reads a video's vibe. 4. Built the matching logic. 5. Built the message writer. 6. Connected everything into one smooth flow you can watch happen live. 7. Built the results page. 8. Tested it and fixed what broke. 9. Put the code on GitHub for anyone to see. 10. Put the website live on the internet."]

## Part 6: Testing (how we know it actually works)

[TO FILL IN: explain in plain language what "QA" (quality assurance, or testing) means. It's pretending to be a real visitor and clicking through the whole thing on purpose to catch anything broken before a real person, like a recruiter, sees it. Note the automated browser-testing tool ("gstack") used to click through the site end to end. List what was checked and confirmed working.]
